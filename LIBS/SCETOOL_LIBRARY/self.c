...
static void __stdcall _get_shdr_flags(s8 *str, u64 flags)
{
	memset(str, '-', 3);
	str[3] = 0;
...
static void __stdcall _get_phdr_flags(s8 *str, u64 flags)
{
	memset(str, '-', 3);
	str[3] = 0;
...
void __stdcall _print_self_header(FILE *fp, self_header_t *h)
{
...
void __stdcall _print_app_info(FILE *fp, app_info_t *ai)
{
	const s8 *name;
...
void __stdcall _print_section_info_header(FILE *fp)
{
	fprintf(fp, "[*] Section Infos:\n");
	fprintf(fp, " Idx Offset   Size     Compressed unk0     unk1     Encrypted\n");
}

void __stdcall _print_section_info(FILE *fp, section_info_t *si, u32 idx)
{
...
void __stdcall _print_sce_version(FILE *fp, sce_version_t *sv)
{
...
void __stdcall _print_control_info(FILE *fp, control_info_t *ci)
{
	const s8 *name;
...
static void __stdcall _print_cap_flags_flags(FILE *fp, oh_data_cap_flags_t *cf)
{
	if(cf->flags & 0x01)
		fprintf(fp, "0x01 ");
	if(cf->flags & 0x02)
...
void __stdcall _print_opt_header(FILE *fp, opt_header_t *oh)
{
	const s8 *name;
...
void __stdcall _print_elf32_ehdr(FILE *fp, Elf32_Ehdr *h)
{
	const s8 *name;
...
void __stdcall _print_elf64_ehdr(FILE *fp, Elf64_Ehdr *h)
{
	const s8 *name;
...
void __stdcall _print_elf32_shdr_header(FILE *fp)
{
	fprintf(fp, "[*] ELF32 Section Headers:\n");
	fprintf(fp, " Idx Name Type          Flags Address Offset Size  ES Align LK\n");
}

void __stdcall _print_elf32_shdr(FILE *fp, Elf32_Shdr *h, u32 idx)
{
	const s8 *name;
	s8 flags[4];
...
void __stdcall _print_elf64_shdr_header(FILE *fp)
{
	fprintf(fp, "[*] ELF64 Section Headers:\n");
	fprintf(fp, " Idx Name Type          Flags Address    Offset   Size     ES   Align    LK\n");
}

void __stdcall _print_elf64_shdr(FILE *fp, Elf64_Shdr *h, u32 idx)
{
	const s8 *name;
	s8 flags[4];
...
void __stdcall _print_elf32_phdr_header(FILE *fp)
{
	fprintf(fp, "[*] ELF32 Program Headers:\n");
	fprintf(fp, " Idx Type     Offset VAddr PAddr FileSize MemSize Flags Align\n");
}

void __stdcall _print_elf32_phdr(FILE *fp, Elf32_Phdr *h, u32 idx)
{
	const s8 *name;
...
void __stdcall _print_elf64_phdr_header(FILE *fp)
{
	fprintf(fp, "[*] ELF64 Program Headers:\n");
	fprintf(fp, " Idx Type     Offset   VAddr    PAddr    FileSize MemSize  PPU SPU RSX Align\n");
}

void __stdcall _print_elf64_phdr(FILE *fp, Elf64_Phdr *h, u32 idx)
{
	const s8 *name;
...
/*
BOOL __stdcall self_print_info(FILE *fp, sce_buffer_ctxt_t *ctxt)
{
	u32 i, self_type;
	const u8 *eident;
...
BOOL __stdcall self_write_to_elf(sce_buffer_ctxt_t *ctxt, const s8 *elf_out)
{
	FILE *fp = NULL;
	u32 i, self_type = 0;
	Elf32_Phdr *ph = NULL;
	const u8 *eident = NULL;
	u8 *data = NULL;
	metadata_section_header_t *msh = NULL;
	Elf64_Phdr *ph64 = NULL;
	...
	if(self_type == SELF_TYPE_LDR || self_type == SELF_TYPE_ISO || eident[EI_CLASS] == ELFCLASS32)
	{
		...
		*/
		...
		ph = (Elf32_Phdr *)(ctxt->scebuffer + ctxt->self.selfh->phdr_offset);
	...
	else
	{
		...
		//Write program headers.
		ph64 = (Elf64_Phdr *)(ctxt->scebuffer + ctxt->self.selfh->phdr_offset);
...
static BOOL __stdcall _create_control_infos(sce_buffer_ctxt_t *ctxt, self_config_t *sconf)
{
	control_info_t *ci = NULL;
	u32 self_type = ctxt->self.ai->self_type;
	ci_data_flags_t *cif = NULL;
	ci_data_digest_40_t *cid = NULL;
	ci_data_npdrm_t *cinp = NULL;
	...
	case SELF_TYPE_NPDRM: //TODO: <-- figure more out.
		{
			...
			cif = (ci_data_flags_t *)((u8 *)ci + sizeof(control_info_t));
	...
	case SELF_TYPE_NPDRM:
		{
			...
			cid = (ci_data_digest_40_t *)((u8 *)ci + sizeof(control_info_t));
	...
	case SELF_TYPE_NPDRM:
		{
			...
			cinp = (ci_data_npdrm_t *)((u8 *)ci + sizeof(control_info_t));
...
static void __stdcall _set_cap_flags(u32 self_type, oh_data_cap_flags_t *capf)
{
	switch(self_type)
	{
	case SELF_TYPE_LV0:
...
static BOOL __stdcall _create_optional_headers(sce_buffer_ctxt_t *ctxt, self_config_t *sconf)
{
	opt_header_t *oh;
	u32 self_type = ctxt->self.ai->self_type;
	oh_data_cap_flags_t *capf = NULL;
	u8 *is = NULL;
	...
	case SELF_TYPE_NPDRM:
		{
			...
			capf = (oh_data_cap_flags_t *)((u8 *)oh + sizeof(opt_header_t));
...
static void __stdcall _fill_sce_version(sce_buffer_ctxt_t *ctxt)
{
...
static void __stdcall _add_phdr_section(sce_buffer_ctxt_t *ctxt, u32 p_type, u32 size, u32 idx)
{
...
static BOOL __stdcall _add_shdrs_section(sce_buffer_ctxt_t *ctxt, u32 idx)
{
...
static BOOL __stdcall _build_self_32(sce_buffer_ctxt_t *ctxt, self_config_t *sconf)
{
	u32 i = 0;
	...
	for(i = 0; i < ehdr->e_phnum; i++)
	{
		...
		sec = _memdup(ctxt->makeself->elf + phdrs[i].p_offset, phdrs[i].p_filesz);
...
static BOOL __stdcall _build_self_64(sce_buffer_ctxt_t *ctxt, self_config_t *sconf)
{
	u32 i = 0;
	u32 loff = 0;
	u32 skip = 0;
	...
	for(i = 0; i < ehdr->e_phnum; i++)
	{
		...
		{
			sec = _memdup(ctxt->makeself->elf + phdrs[i].p_offset, (u32)phdrs[i].p_filesz);
...
BOOL __stdcall self_build_self(sce_buffer_ctxt_t *ctxt, self_config_t *sconf)
{
	//const u8 *eident;
