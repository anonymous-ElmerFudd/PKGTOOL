...
void __stdcall _print_sce_header(FILE *fp, sce_header_t *h)
{
	const s8 *name;
...
void __stdcall _print_metadata_info(FILE *fp, metadata_info_t *mi)
{
...
void __stdcall _print_metadata_header(FILE *fp, metadata_header_t *mh)
{
...
static void __stdcall _print_metadata_section_header_header(FILE *fp)
{
	fprintf(fp, "[*] Metadata Section Headers:\n");
	fprintf(fp, " Idx Offset   Size     Type Index Hashed SHA1 Encrypted Key IV Compressed\n");
}

void __stdcall _print_metadata_section_header(FILE *fp, metadata_section_header_t *msh, u32 idx)
{
...
void __stdcall _print_sce_file_keys(FILE *fp, sce_buffer_ctxt_t *ctxt)
{
	u32 i;
...
static sce_buffer_ctxt_t* __stdcall _sce_create_ctxt()
{
...
sce_buffer_ctxt_t* __stdcall sce_create_ctxt_from_buffer(u8 *scebuffer)
{
	...
	switch(res->sceh->header_type)
	{
		case SCE_HEADER_TYPE_SELF:
		{
			...
			//SCE version.
			if((void*)res->self.selfh->sce_version_offset != NULL)
			{
			...
			len = (u32)res->self.selfh->control_info_size;
			if(len > 0)
			{
...
//	LIST_FOREACH(iter, ctxt->secs)
	for(iter = ctxt->secs->head; iter != NULL; iter = iter->next)
	{
...
static u32 __stdcall _sce_get_ci_len(sce_buffer_ctxt_t *ctxt)
{
	u32 res = 0;
	lnode_t* iter = NULL;
...
//	LIST_FOREACH(iter, ctxt->self.cis)
	for(iter = ctxt->self.cis->head; iter != NULL; iter = iter->next)
		res += ((control_info_t *)iter->value)->size;

	return res;
}

static u32 __stdcall _sce_get_oh_len(sce_buffer_ctxt_t *ctxt)
{
	u32 res = 0;
	lnode_t* iter = NULL;

//	LIST_FOREACH(iter, ctxt->self.ohs)
	for(iter = ctxt->self.ohs->head; iter != NULL; iter = iter->next)
		res += ((opt_header_t *)iter->value)->size;

	return res;
}

void __stdcall _sce_fixup_ctxt(sce_buffer_ctxt_t *ctxt)
{
	u32 i = 0;
	u32 base_off = 0;
	u32 last_off = 0;
	lnode_t* iter = NULL;
	sce_section_ctxt_t *sec = NULL;
...
//	LIST_FOREACH(iter, ctxt->secs)
	for(iter = ctxt->secs->head; iter != NULL; iter = iter->next)
	{
		...
		//Section offsets.
		sec = (sce_section_ctxt_t *)iter->value;
		sec->offset = base_off;
...
void __stdcall _sce_fixup_keys(sce_buffer_ctxt_t *ctxt)
{
	u32 i = 0;
...
static void __stdcall _sce_build_header(sce_buffer_ctxt_t *ctxt)
{
	u32 i = 0;
	u32 ci_base = 0;
	opt_header_t *oh = NULL;
	lnode_t* iter = NULL;
...
static BOOL __stdcall _sce_sign_header(sce_buffer_ctxt_t *ctxt, keyset_t *ks)
{
	u8 hash[0x14];
...
static void __stdcall _sce_calculate_hashes(sce_buffer_ctxt_t *ctxt)
{
	u32 i = 0, sha1_idx;
	lnode_t* iter = NULL;
...
static BOOL __stdcall _sce_encrypt_header(sce_buffer_ctxt_t *ctxt, u8 *keyset)
{
	u8 *ptr;
	size_t nc_off;
...
static void __stdcall _sce_encrypt_data(sce_buffer_ctxt_t *ctxt)
{
	u32 i = 0;
	aes_context aes_ctxt;
	lnode_t* iter = NULL;
...
BOOL __stdcall sce_decrypt_header(sce_buffer_ctxt_t *ctxt, u8 *metadata_info, u8 *keyset)
{
	...
	if(ctxt->sceh->header_type == SCE_HEADER_TYPE_SELF)
	{
		...
		oh = (opt_header_t *)(ctxt->keys + ctxt->metah->key_count * 0x10);
		_es_opt_header(oh);
	...
	{		
		...
		if(ctxt->metash[i].encrypted == METADATA_SECTION_ENCRYPTED)
		{
			...
			else
			{
				memcpy(iv, ctxt->keys + ctxt->metash[i].iv_index * 0x10, 0x10);
				aes_setkey_enc(&aes_ctxt, ctxt->keys + ctxt->metash[i].key_index * 0x10, 128);
				ptr = ctxt->scebuffer + ctxt->metash[i].data_offset;
