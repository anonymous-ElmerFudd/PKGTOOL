...
static __inline void _es_elf32_ehdr(Elf32_Ehdr *h)
{
...
static __inline void _copy_es_elf32_ehdr(Elf32_Ehdr *dst, Elf32_Ehdr *src)
{
	memcpy(dst, src, sizeof(Elf32_Ehdr)); 
	_es_elf32_ehdr(dst);
}

static __inline void _es_elf64_ehdr(Elf64_Ehdr *h)
{
	h->e_type = _ES16(h->e_type);
	h->e_machine = _ES16(h->e_machine);
	h->e_version = _ES32(h->e_version);
...
static __inline void _copy_es_elf64_ehdr(Elf64_Ehdr *dst, Elf64_Ehdr *src)
{
	memcpy(dst, src, sizeof(Elf64_Ehdr)); 
	_es_elf64_ehdr(dst);
}

static __inline void _es_elf32_shdr(Elf32_Shdr *h)
{
...
static __inline void _copy_es_elf32_shdr(Elf32_Shdr *dst, Elf32_Shdr *src)
{
	memcpy(dst, src, sizeof(Elf32_Shdr)); 
	_es_elf32_shdr(dst);
}

static __inline void _es_elf64_shdr(Elf64_Shdr *h)
{
	h->sh_name = _ES32(h->sh_name);
	h->sh_type = _ES32(h->sh_type);
...
static __inline void _copy_es_elf64_shdr(Elf64_Shdr *dst, Elf64_Shdr *src)
{
	memcpy(dst, src, sizeof(Elf64_Shdr)); 
	_es_elf64_shdr(dst);
}

static __inline void _es_elf32_phdr(Elf32_Phdr *h)
{
...
static __inline void _copy_es_elf32_phdr(Elf32_Phdr *dst, Elf32_Phdr *src)
{
	memcpy(dst, src, sizeof(Elf32_Phdr)); 
	_es_elf32_phdr(dst);
}

static __inline void _es_elf64_phdr(Elf64_Phdr *h)
{
	h->p_type = _ES32(h->p_type);
	h->p_flags = _ES32(h->p_flags);
...
static __inline void _copy_es_elf64_phdr(Elf64_Phdr *dst, Elf64_Phdr *src)
{
